...
@RestrictRole("admin")
@View(value = "Components", url="component", icon="glyphicon glyphicon-th") @Order(4)
public class ComponentTest {
	
	@Order(2) @HelloWorld(heading="#{generic.title}") private String value = "Yo mang, what's up?";

	@Order(11) @Breadcrumb({"Home", "Water", "Swimming"}) Void b;
		
	@Order(6) @DescriptionList("#{components.desc.list.one}") Void d;

	@Order(3) @Image("${me.randomImage()}") Void i;
	public String randomImage(){
		return "sweetsugar.png";
	}
	
	@Order(15) @Html("#{components.desc.h1.heading}") Void h1;

	@Order(14) @Html("#{components.desc.h2.heading}") Void h2;

	@Order(13) @Html("#{components.desc.h3.heading}") Void h3;

	@Order(12) @Html("#{components.desc.h4.heading}") Void h4;

	@Order(9) @Html("#{components.desc.h5.heading}") Void h5;

	@Order(8) @Html("#{components.desc.h6.heading}") Void h6;

	@Order(16) @Paragraph("#{components.paragraph}") Void p;

	@Order(27) @Paragraph(value = "#{components.paragraph}",
			style = ParagraphStyle.PARAGRAPH_LEAD, textStyle = TextStyle.DANGER) Void p2;

	@Order(26) @Paragraph(value = "#{components.paragraph}",
			textStyle = TextStyle.MUTED) Void p3;

	@Order(24) @Paragraph(value = "#{components.paragraph}",
			textStyle = TextStyle.INFO) Void p4;

	@Order(23) @Paragraph(value = "#{components.paragraph}",
			textStyle = TextStyle.WARNING) Void p5;

	@Order(25) @Paragraph(value = "#{components.paragraph}",
			backgroundStyle = BackgroundStyle.INFO) Void p6;

	@Order(1) @Address({ "Sugar Framework Inc.", "12345 Internet Way, Suite 45", "San Francisco, CA 94107", "P: (123) 456-7890" }) 
	 Void address;

	@Order(17) @BlockQuote({"#{components.blockquote.message}",
				 "#{components.blockquote.source}"}) Void blockQuote;

	@Order(21) @BlockQuote(value={ "#{components.blockquote.message}" }, reverse = true) Void blockQuote2;

	@Order(18) @Descriptor({"#{components.descriptor.title}", "#{components.descriptor}"})Void d1;
	
	@Order(20) @PrettyDescriptor(value={"#{components.descriptor.title}", "#{components.descriptor}"},
					  style = Style.STYLE_WARNING) Void d2;

	@Order(22) @UnorderedList(style = ListStyle.INLINE)
	...
	@Order(5) @UnorderedList(style = ListStyle.UNSTYLED)
	...
	@Order(19) @SugarLabel(value="#{components.label}", style = Style.STYLE_DANGER) Void l;
	...
	@Order(10) @Action("Lets test") public void testBean( 
